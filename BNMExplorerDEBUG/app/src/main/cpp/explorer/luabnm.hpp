...
static void GenericHook_ReturnVoid(void* inst) { return; }

static int l_cls_patch_void(lua_State* L) {
    auto* c = (LuaClass*)luaL_checkudata(L, 1, MT_CLS);
    const char* mname = luaL_checkstring(L, 2);
    ...
    if (cls.IsValid()) {
        ...
        if (mb.IsValid()) {
            void* oldPtr = nullptr;
            BNM::InvokeHook(mb, (void*)GenericHook_ReturnVoid, &oldPtr);
        }
    }
    return 0;
}
...
static int l_cls_callmethod(lua_State* L) {
    ...
    // Ignorăm argumentul 'self' dacă a fost trimis via sintaxa ":"
    int argOffset = 0;
    if (nargs > 0 && lua_isuserdata(L, 1)) {
        if (lua_touserdata(L, 1) == c) {
            argOffset = 1;
            nargs--;
        }
    }
    ...
    for (int i = 0; i < nargs; i++) {
        ...
        // Citim argumentele decalat dacă există 'self'
        runtimeArgs.push_back(luaToArg(L, i + 1 + argOffset, pt, vi, vf, vd, vb, vi64));
    }
...
static int l_cls_index(lua_State* L) {
    ...
    if (strcmp(key, "PatchReturnVoid") == 0) { lua_pushcfunction(L, l_cls_patch_void); return 1; }
